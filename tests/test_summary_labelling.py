import httpx
from taranis_ds import summary_labelling
import taranis_ds.summary_labelling
...
def test_convert_language():
    assert summary_labelling.convert_language("fr") == "french"
    assert summary_labelling.convert_language("de") == "german"
    assert summary_labelling.convert_language("en") == "english"
    assert summary_labelling.convert_language("ru") == "russian"


@patch("taranis_ds.summary_labelling.time.sleep")
@patch("taranis_ds.summary_labelling.create_chain")
def test_create_summaries_for_news_items(mock_create_chain, mock_sleep, results_db):
    ...
    summary_labelling.create_summaries_for_news_items(chat_model, news_items, results_db, "results", 300, 0.5, 1.)
    ...
    summary_labelling.create_summaries_for_news_items(chat_model, news_items, results_db, "results", 300, 0.8, 1.)
    saved_results = results_db.execute("SELECT summary, summary_status FROM results WHERE id='1'").fetchall()
